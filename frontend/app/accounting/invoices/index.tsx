...
export default function InvoicesList() {
  ...
  const remove = (inv: any) => Alert.alert(
    'Delete Invoice?', `Delete ${inv.invoice_no}? This cannot be undone.`,
    [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: async () => {
      try { await api.del(`/invoices/${inv.id}`); await load(); } catch (e: any) { Alert.alert('Failed', e?.message || 'Try again'); }
    } }]
  );
